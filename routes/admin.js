...
router.get('/catalog-quality', (req, res) => {
  try {
    ...
    res.json({
      data: {
        summary: mapAdminCatalogQualitySummary(summary, missingGenres, missingServices),
        by_type: byTypeRows.map(mapAdminCatalogQualityTypeRow),
        recently_updated: recentTitlesRows.map(mapAdminCatalogQualityRecentTitleRow),
      },
    });
  } catch (error) {
